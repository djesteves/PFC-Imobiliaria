 ...
 */
package org.example.pfc.controle.ajax;
...
import org.example.pfc.dao.AgendamentoDAO;
import org.example.pfc.dao.ImovelDAO;
import org.example.pfc.dao.RelatorioDAO;
 ...
 * @author Diieg
 */
@WebServlet(name = "ControleServ", urlPatterns = { "/ImovelListarAprovados", "/VisualizarImovel", "/ListarCorretores",
        "/EmitirRelatorio" })
@MultipartConfig
public class ControleServ extends HttpServlet {
    ...
    public void listarCorretores(HttpServletResponse response) throws Exception {
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println(
                    "<option value=\"AutoAlocar\">Alocar um corretor automaticamente/Não tenho preferência</option>");
    ...
    public void listarImovelPorID(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        try (PrintWriter out = response.getWriter()) {
            out.println("<div class=\"modal-header\">\n"
                    + " <h5 class=\"modal-title\" id=\"modalImovelLabel\"> " + im.getTitulo() + " - Número do Anúncio: "
                    + im.getId_imovel() + "</h5>\n"
            ...
            out.println("<div class=\"modal-body\">"
                    ...
                    + "       <img style=\"cursor: zoom-in\" class=\"img-fluid rounded\" src=\""
                    + request.getContextPath() + "/upload/" + im.getDiretorio_imagem() + "\"> </img>\n"
                    + "   </div>\n"
                    + "   <div class=\"col-lg-6\">\n"
                    + "<div class=\"card \">\n"
                    + "<div class=\"card-body \">\n"
                    ...
                    + "  <p><i class=\"fas fa-money-check-alt\"></i> " + nf.format(im.getValor()) + " Valor " + tpvenda
                    + "</p>\n"
                    + "  <p><i class=\"fas fa-file-invoice-dollar\"></i> " + nf.format(im.getIptu())
                    + " Valor do IPTU/Anual</p>\n"
                    + "  <p><i class=\"fas fa-file-invoice-dollar\"> </i> " + nf.format(im.getCondominio())
                    + " Valor do Condominio</p>\n"
                    + "  <hr class=\"my-4\">\n"
                    + "<div class=\"text-center \">\n"
                    + " <form action=\"" + request.getContextPath()
                    + "/usuario/AgendamentoSolicitar.jsp\" method=\"post\">"
                    + " <input type=\"hidden\" id=\"id_imovel\" name=\"id_imovel\" value=\"" + im.getId_imovel()
                    + "\" >"
                    + " <input type=\"hidden\" id=\"emailanunciante\" name=\"emailanunciante\" value=\""
                    + im.getUsuario().getEmail() + "\" >"
    ...
    public void emitirRelatorio(HttpServletRequest request, HttpServletResponse response) throws Exception {
        ...
        InputStream jrxml = getClass().getResourceAsStream("/relatorios/" + relatorio + ".jrxml");
        String path = contexto.getRealPath("resultados/");

        parametros.put("logo", contexto.getRealPath("assets/img/icon_imob.png"));
        parametros.put("path", path);

        if (relatorio.equalsIgnoreCase("ImoveisCadastradosPeriodo")
                || relatorio.equalsIgnoreCase("UsuariosCadastradosPeriodo")
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            if (imoveis.isEmpty()) {
                ...
                for (Imovel im : imoveis) {
                    ...
                    out.println("<img class=\"card-img-top\" src=\"" + request.getContextPath() + "/upload/"
                            + im.getDiretorio_imagem() + "\" alt=\"Imagem do Imóvel\" height=\"225\" width=\"210\">");
                    ...
                    out.println("<button type=\"button\" class=\"btn btn-primary\" onClick=\"carregarImovel('"
                            + im.getId_imovel() + "')\" data-toggle=\"modal\" data-target=\"#modalImovel\">\n"
                            + "        Detalhes\n"
                            + "    </button>");
